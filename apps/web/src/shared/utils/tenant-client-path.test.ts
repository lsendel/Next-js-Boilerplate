...
describe("resolveTenantClientPath", () => {
  ...
  it("prefixes locale for non-default languages", () => {
    expect(
      resolveTenantClientPath("/", { context: sharedContext, locale: "fr" }),
    ).toBe("/fr");
    expect(
      resolveTenantClientPath("/pricing", {
  ...
  it("prefixes slug for shared-domain tenants", () => {
    ...
    expect(resolveTenantClientPath("/pricing", { context })).toBe(
      "/acme/pricing",
    );
  });
  ...
  it("preserves both slug and locale ordering", () => {
    ...
    expect(resolveTenantClientPath("/pricing", { context, locale: "fr" })).toBe(
      "/acme/fr/pricing",
    );
  });
