...
describe("Format Utilities", () => {
  ...
  describe("formatNumber", () => {
    ...
    it("should handle custom format options", () => {
      ...
      expect(formatNumber(1234.5, "en-US", { minimumFractionDigits: 2 })).toBe(
        "1,234.50",
      );

      expect(formatNumber(0.123, "en-US", { style: "percent" })).toBe("12%");
    });
    ...
    it("should handle very small numbers", () => {
      ...
      expect(
        formatNumber(0.123456, "en-US", { maximumFractionDigits: 2 }),
      ).toBe("0.12");
    });
  });
  ...
  describe("slugify", () => {
    ...
    it("should create valid URL slugs", () => {
      ...
      expect(slugify("JavaScript Tips & Tricks")).toBe(
  ...
  describe("Integration tests", () => {
    ...
    it("should format e-commerce product display", () => {
      ...
      expect(capitalize(productName.toLowerCase())).toBe("Premium widget 2000");
      expect(formatCurrency(price, "USD", "en-US")).toBe("$299.99");
      // truncate takes first 30 chars, then adds "..."
      expect(truncate(description, 30)).toBe(
        "This is an amazing product wit...",
      );
    });
  });
