...
| --- | --- | --- | --- |
| `DATABASE_URL` | Server | CI + any Node-based hosting | Must point to a Postgres-compatible instance (PGlite/PostgreSQL) for Node workflows (tests, `db:migrate`). Cloudflare D1 in production uses the `DB` binding configured via Wrangler instead of this variable. |
...
| Variable | Scope | Where | Notes |
| --- | --- | --- | --- |
| `SENTRY_DSN`, `SENTRY_ORGANIZATION`, `SENTRY_PROJECT`, `SENTRY_AUTH_TOKEN` | Mixed | Hosting + GitHub secrets | Required to upload source maps and capture runtime errors. |
| `NEXT_PUBLIC_POSTHOG_KEY`, `NEXT_PUBLIC_POSTHOG_HOST` | Client | Hosting | Enable analytics dashboarding if you use PostHog. |
| `NEXT_PUBLIC_ENABLE_SENTRY`, `NEXT_PUBLIC_ENABLE_POSTHOG`, `NEXT_PUBLIC_ENABLE_CF_ANALYTICS` | Client | GitHub repo/env vars or Cloudflare Pages env vars | Feature flags to turn monitoring providers on/off per environment. |
