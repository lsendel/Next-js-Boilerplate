...
Monitoring providers are controlled via the feature flags `NEXT_PUBLIC_ENABLE_SENTRY`, `NEXT_PUBLIC_ENABLE_POSTHOG`, and `NEXT_PUBLIC_ENABLE_CF_ANALYTICS`. For the per-environment matrix and CI wiring, see `docs/ops/observability.md` in the repository.
